...
try {
    $stmt = $pdo->prepare("SELECT * FROM investigations WHERE id = ?");
    $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $sql = "UPDATE investigations SET patient_id = ?, test_type = ?, result = ?, status = ? WHERE id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$patient_id, $test_type, $result, $status, $id]);
        
        header("Location: investigation.php?success=Lab test updated successfully!");
        exit();
    } catch(PDOException $e) {
        $error = $e->getMessage();
    }
...
<head>
    ...
    <title>Edit Lab Test - SmartHealth</title>
                        ...
                        <form method="POST">
                            <div class="mb-3">
                                ...
                                <select name="patient_id" class="form-control" required>
                                    ...
                                    ?>
                                    <option value="<?= $p['id'] ?>" <?= $p['id'] == $investigation['patient_id'] ? 'selected' : '' ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="test_type" class="form-control" value="<?= htmlspecialchars($investigation['test_type']) ?>" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Result</label>
                                <textarea name="result" class="form-control" rows="3"><?= htmlspecialchars($investigation['result'] ?? '') ?></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Status</label>
                                <select name="status" class="form-control" required>
                                    <option value="Pending" <?= $investigation['status'] == 'Pending' ? 'selected' : '' ?>>Pending</option>
                                    <option value="Completed" <?= $investigation['status'] == 'Completed' ? 'selected' : '' ?>>Completed</option>
                                </select>
                            </div>
                            <button type="submit" class="btn btn-primary">Update Test</button>
                            <a href="investigation.php" class="btn btn-secondary">Cancel</a>
                        </form>
                    </div>
                </div>
            </main>
