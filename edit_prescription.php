...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $sql = "UPDATE prescriptions SET patient_id = ?, medication = ?, dosage = ?, instructions = ? WHERE id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$patient_id, $medication, $dosage, $instructions, $id]);
        
        header("Location: Prescription.php?success=Prescription updated successfully!");
        exit();
    } catch(PDOException $e) {
        $error = $e->getMessage();
    }
...
<head>
    ...
    <title>Edit Prescription - SmartHealth</title>
                        ...
                        <form method="POST">
                            <div class="mb-3">
                                ...
                                <select name="patient_id" class="form-control" required>
                                    ...
                                    ?>
                                    <option value="<?= $p['id'] ?>" <?= $p['id'] == $prescription['patient_id'] ? 'selected' : '' ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="medication" class="form-control" value="<?= htmlspecialchars($prescription['medication']) ?>" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Dosage</label>
                                <input type="text" name="dosage" class="form-control" value="<?= htmlspecialchars($prescription['dosage']) ?>" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Instructions</label>
                                <textarea name="instructions" class="form-control" rows="3"><?= htmlspecialchars($prescription['instructions'] ?? '') ?></textarea>
                            </div>
